...
class Stage_WP_Plugin_Manager {
	...
	private $network_stage_plugins     = array();
	 ...
	 */
	private function get_managed_plugins( $option = '', $filter = '' ) {
		$option = $option ? $option : $this->wp_option;
		$filter = $filter ? $filter : 'stage_wp_plugin_manager_managed_plugins'; 
		$managed_plugins = ( $plugins = get_option( $option ) ) 
		                   ? $plugins : array();
		foreach ( $managed_plugins as $key => $plugins ) {
			foreach ( $plugins as $_key => $plugin ) {
				if (   plugin_basename( dirname( __FILE__ ) . '/stage-wp-plugin-manager.php' ) 
					== $plugin
				) {
