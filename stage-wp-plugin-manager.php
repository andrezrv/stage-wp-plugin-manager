 ...
 * stage only. You can activate and deactivate plugins separatedly for
 ...
 * Description:  Gives you the option to define which plugins must be active for a particular stage only. You can activate and deactivate plugins separatedly for local, staging and production stages. To use this plugin, you need to add a constant named <code>WP_ENV_PLUGINS</code> to your <code>wp-config.php</code> file, with one of the following values: <code>local</code>, <code>staging</code>, <code>production</code>. This plugin supports WordPress Multisite, and can manage network activated plugins.
 * Author: Andr&eacute;s Villarreal
 * Author URI: http://about.me/andrezrv
 * Version: 1.0-RC
 */
require( dirname( __FILE__ ) . '/stage-wp-plugin-manager.class.php' );
require( dirname( __FILE__ ) . '/stage-wp-plugin-manager-loader.class.php' );
new Stage_WP_Plugin_Manager_Loader;
